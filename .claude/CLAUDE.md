...
| `tool-examples.md` | Tool selection examples and input patterns |
| `tool-response-filtering.md` | Tool response filtering and output optimization |
...
> **LLM Routing:** `.claude/llm-routing.md` — carregado em toda sessão. Define roteamento automático de modelos por agente e complexidade de tarefa.
...
<!-- AIOX-MANAGED-START: llm-routing -->
## LLM Routing — Seleção Automática de Modelos

O AIOX aplica roteamento automático de LLMs para otimizar custo/qualidade. As regras completas estão em **`.claude/llm-routing.md`** (carregado em toda sessão).

### Matriz de Roteamento por Agente

| Tier | Agentes | Modelo Padrão |
|------|---------|---------------|
| **TIER-3 Premium** | `@architect`, `@aiox-master` | `opus` |
| **TIER-2 Standard** | `@dev`, `@qa`, `@pm`, `@analyst`, `@data-engineer` | `sonnet` |
| **TIER-1 Economy** | `@po`, `@sm`, `@ux-design-expert`, `@devops` | `haiku` |

### Heurística de Complexidade (prevale sobre tier do agente)

| Nível | Indicadores | Modelo |
|-------|------------|--------|
| SIMPLE | < 100 palavras, 1 arquivo, "fix/typo/rename/list/read/show" | `haiku` |
| MEDIUM | 2-5 arquivos, "implement/create/build/test/review" | `sonnet` |
| COMPLEX | 6+ arquivos, "architect/migrate/security/debug unknown" | `opus` |

### Subagentes (Task tool)

Ao spawnar subagentes, SEMPRE definir o parâmetro `model`:
- Explore/Bash agents → `haiku`
- Plan agents → `sonnet`
- General-purpose → tier do agente na matriz acima

### Regras de Comportamento

- **NUNCA** trocar modelo automaticamente — apenas sugerir (`/model sonnet`)
- Sugerir no máximo 1 vez por categoria de tarefa na sessão
- Respeitar preferência explícita do usuário sem novas sugestões

> **Documento completo:** `.claude/llm-routing.md`
<!-- AIOX-MANAGED-END: llm-routing -->
